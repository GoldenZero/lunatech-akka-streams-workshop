...
class HelloWordSpec extends FreeSpec {
  "having actor system and materializer should" - {
    implicit val system = ActorSystem("HelloSpec")
    implicit val materializer = ActorMaterializer()

    "runForeach with a source of a range" in {
    ...
    "get notification on stream termination" in {
      ...
      // you should never use global context in real life
    ...
    "construct a full fledged source to sink pipeline to run" in {
      ...
      val source: Source[Int, NotUsed] = Source(1 to 10)
      val sink: Sink[Int, Future[Done]] = Sink.foreach(i => println(i))
      val pipeline: RunnableGraph[NotUsed] = source.via(flow).to(sink)
      pipeline.run()
    }
